...
private:
  ...
  static const size_t LAYERS = 15;
  static const size_t HIDDEN_LAYER_DIM = 28;
  ...
  void initNetwork() {
    ...
    for (size_t i = 0; i < LAYERS - 1; i++) {
      ...
      weights[i] = arma::Mat<float>(neuronVecDims[i + 1], neuronVecDims[i],
                                    arma::fill::randn) *
                   std::sqrt(1.0f / neuronVecDims[i]);

      biases[i] = arma::Col<float>(neuronVecDims[i + 1], arma::fill::zeros);

      weightChanges[i] = arma::Mat<float>(neuronVecDims[i + 1],
                                          neuronVecDims[i], arma::fill::zeros);
      biasChanges[i] =
...
public:
  NeuralNet(const char **files)
      : weights(LAYERS - 1), biases(LAYERS - 1), weightChanges(LAYERS - 1),
        biasChanges(LAYERS - 1), neuronsA(LAYERS), neuronsZ(LAYERS) {
  ...
  void feedForward() {
    for (size_t i = 0; i < LAYERS - 1; i++) {
      ...
      neuronsA[i + 1].transform(
          [](float val) { return 1.0 / (1.0f + std::exp(-val)); });
    }
  }
  ...
  arma::Col<float> layerGrad(const arma::Col<float> &errorL,
    ...
    arma::Col<float> currLayerError =
        errorL %
        (neuronsA[layerNum] % (1 - arma::Col<float>(neuronsA[layerNum])));
  ...
  void placeImageInNet(size_t imageNum, std::vector<uint8_t> &imgsRef) {
    std::vector<uint8_t> individualImage(imgsRef.begin() + imageNum * numInputs,
                                         imgsRef.begin() +
                                             imageNum * numInputs + numInputs);
    neuronsZ[0] = arma::conv_to<arma::Mat<float>>::from(individualImage);
    neuronsZ[0] = neuronsZ[0] %
                  arma::Col<float>(numInputs, arma::fill::value(1.0f / 255));
    neuronsA[0] = neuronsZ[0];
  }

  template <int batchSize> void train() {
    size_t steps = 100000;
  ...
  float evalNetwork() {
    ...
    for (; i < 10000; i++) {
      ...
      diff += arma::accu((this->neuronsA[LAYERS - 1] - goal) %
  ...
  void guess() {
    ...
    for (int j = 0; j < 5; j++) {
      ...
      for (int i = input[j] * 28 * 28; i < (input[j] + 1) * 28 * 28; i++) {
        if (i % 28 == 27)
          std::printf("%c", '\n');
        this->testImgs[i] < 0.4 ? std::printf("%s", ". ")
                                : std::printf("%s", "# ");
      }
      this->feedForward();
      std::printf("%s", "\n----------------------------\n");
      ...
      std::printf("%s", "Goal: ");
      std::printf("%i", this->testLabels[input[j]]);
      std::printf("%c", '\n');

      int maxIndex = 0;
      for (int i = 1; i < this->neuronsA[LAYERS - 1].n_elem - 1; i++) {
        if (this->neuronsA[LAYERS - 1][i] >
            this->neuronsA[LAYERS - 1][maxIndex])
          maxIndex = i;
      }

      std::printf("%s", "Guess: ");
      std::printf("%i", maxIndex);
      std::printf("%c", '\n');
    }
  }
};
